...
files = glob('/data2/zhousiyu/dataset/kid/*.jpg')
...
def run(file):
    ...
    '''save txt'''
    txt_name = 'kid_txt/' + file.split('/')[-1].split('.')[0] + '.txt'
    f = open(txt_name, 'w')
    f.write(content)
    f.close()
    ...
    landmark = cv2.resize(landmark,(224, 224),interpolation=cv2.INTER_CUBIC)
    concat = cv2.hconcat((img, landmark))
    
    '''save img'''
    img_name = 'kid_img/' + file.split('/')[-1].split('.')[0] + '.jpg'
